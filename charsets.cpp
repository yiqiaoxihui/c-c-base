...
{
    gchar *str= (gchar *)malloc(length+1);
	str[length]='\0';
    ...
    while (length > 0) {
        ...
        if (ch >0 && ch < 0x80)
			...
			//str[i++]=UNREPL;
			str[i++]=' ';		//非可见类型ascii码使用空格代替，liuyang
        ptr++;
        length--;
    }
...
guint8 * get_ucs_2_string(/*wmem_allocator_t *scope,*/ const guint8 *ptr, gint length, const guint encoding)
{
    ...
    _wmem_strbuf_t *strbuf;
	//gchar *strbuf= (gchar *)malloc(length+1);
    ...
    for(i = 0; i + 1 < length; i += 2) {
        ...
        strbuf_append_unichar(strbuf,length, uchar);
		//strbuf[i]=uchar;
    }
...
gchar * get_ascii_7bits_string(/*wmem_allocator_t *scope,*/ const guint8 *ptr,
                       const gint bit_offset, gint no_of_chars)
{
    //wmem_strbuf_t *strbuf;
	gchar *strbuf = (gchar *) malloc(no_of_chars +1);
	gchar *head_pos=strbuf;
	strbuf[no_of_chars]='\0';
	...
	strbuf[char_count]='\0';
	//content = std::string(head_pos);			//add by liuyang,love string
	//free(strbuf);
    return strbuf;/*(guint8 *)wmem_strbuf_finalize (strbuf)*/
}
